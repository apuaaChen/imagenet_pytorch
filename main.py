...
def main_worker(gpu, ngpus_per_node, args_):
    ...
    # create model
    if args_.pretrained:
        print("=> using pre-trained model '{}'".format(args_.arch))
        model = models.__dict__[args_.arch](pretrained=True)
    else:
        print("=> creating model '{}'".format(args_.arch))
        model = models.__dict__[args_.arch]()
    ...
    if args_.resume:
        if os.path.isfile(args_.resume):
            ...
            if args_.gpu is not None:
                ...
                best_acc1 = best_acc1.to(args_.gpu)
    ...
    for epoch in pbar:
        ...
        # right tensorboard
        result.record(loss, top1_t, top5_t, acc1, acc5, epoch)
