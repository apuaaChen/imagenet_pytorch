...
class Aux(nn.Module):
    ...
    def __init__(self, in_channels, num_classes):
        ...
        self.conv1 = nn.Conv2d(128, 768, kernel_size=((14 + 2) // 3, (14 + 2) // 3))
    ...
    def forward(self, x):
        x = nn.ReLU()(x)
        x = F.avg_pool2d(x, kernel_size=5, stride=3, padding=(2, 2), count_include_pad=False)
        x = self.conv0(x)
        x = self.bn0(x)
        x = self.acf(x)
        x = self.conv1(x)
...
class NASNetAMobile(nn.Module):
    ...
    def __init__(self, num_classes=1001, stem_filters=32, penultimate_filters=1056, filters_multiplier=2):
        ...
        self.fc = nn.Linear(24*filters, self.num_classes)

        # aux
        self.aux = Aux(in_channels=12*filters, num_classes=self.num_classes)
    ...
    def features(self, input):
        ...
        aux_out = self.aux(x_cell_9)

        x_cell_12 = self.cell_12(x_reduction_cell_1, x_cell_9)
        x_cell_13 = self.cell_13(x_cell_12, x_reduction_cell_1)
        x_cell_14 = self.cell_14(x_cell_13, x_cell_12)
        x_cell_15 = self.cell_15(x_cell_14, x_cell_13)
        return x_cell_15, aux_out  # returning the x_reduction_cell1 is to perform the auxiliary classifier
